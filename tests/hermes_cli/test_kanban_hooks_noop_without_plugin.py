...
# _record_task_failure moved here in upstream's September 2026 decomposition;
# everything else this file touches still lives on kanban_db.
import hermes_cli.kanban_db_dispatch as kdbd
...
def test_auto_block_unchanged_without_plugin(monkeypatch):
    ...
    conn.commit()
    assert kdbd._record_task_failure(
