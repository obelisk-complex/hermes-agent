...
class TestInterimAssistantMessageConfig:
    ...
    def test_migrate_to_v15_adds_interim_assistant_message_gate(self, tmp_path):
        ...
        # interim_assistant_messages defaults to True in DEFAULT_CONFIG, so
        # save_config(strip_defaults=True) does not materialise it to disk.
        # Verify via load_config() which deep-merges defaults.
        from hermes_cli.config import load_config
        merged = load_config()
        assert merged["display"]["interim_assistant_messages"] is True
...
class TestDiscordChannelPromptsConfig:
    ...
    def test_migrate_adds_discord_channel_prompts_default(self, tmp_path):
        ...
        # channel_prompts defaults to {} in DEFAULT_CONFIG, so
        # save_config(strip_defaults=True) does not materialise it to disk.
        # Verify via load_config() which deep-merges defaults.
        from hermes_cli.config import load_config
        merged = load_config()
        assert merged["discord"]["channel_prompts"] == {}
...
class TestWriteApprovalMigration:
    ...
    def test_on_and_off_map_to_false(self, tmp_path):
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            ...
            migrate_config(interactive=False, quiet=True)
            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            # write_approval defaults to False in DEFAULT_CONFIG, so
            # save_config(strip_defaults=True) does not materialise it to disk.
            # Verify the old key is gone and the effective value is False.
            assert "write_mode" not in raw.get("memory", {})
            assert "write_mode" not in raw.get("skills", {})
            from hermes_cli.config import load_config
            merged = load_config()
            assert merged["memory"]["write_approval"] is False
            assert merged["skills"]["write_approval"] is False
