...
The ``_aliases_and_data_patch`` fixture is per-test because the patch is
a class-level monkeypatch on ``Vt100Parser``; the autouse fixture in the
sibling test file restores only ``ANSI_SEQUENCES``.
"""
...
def _aliases_and_data_patch():
    """Mirror the CLI startup installs (cli.py:62-70) — alias tables AND
    ...
    # (idempotent re-install returns 0 by design).
    assert getattr(
        vt100_mod.Vt100Parser._call_handler, "_hermes_char_data_normalized", False
