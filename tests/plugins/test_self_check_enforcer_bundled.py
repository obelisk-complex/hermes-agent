...
pre_verify gate) with zero hand-installation. Stdlib-only; runs standalone or
under pytest.
"""
...
def test_bundled_plugin_registers_pre_verify_and_eight_hooks():
    mod = _load()
    ctx = _RecordingContext()
    mod.register(ctx)
    assert "pre_verify" in ctx.registered, f"pre_verify not wired; got {sorted(ctx.registered)}"
...
def test_manifest_declares_the_gate_hooks():
    with open(_YAML, encoding="utf-8") as f:
        manifest = f.read()
    for hook in ("pre_verify", "subagent_stop", "pre_tool_call"):
        assert hook in manifest, f"plugin.yaml does not declare {hook}"


def test_version_is_v380_or_later():
    with open(_PLUGIN, encoding="utf-8") as f:
        head = f.read(400)
    assert "v3.8.0" in head, "bundled plugin docstring should be v3.8.0+"
