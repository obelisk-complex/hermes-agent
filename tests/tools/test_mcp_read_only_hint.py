...
from tools.mcp_tool import MCPServerTask, mcp_tool_read_only_hint  # noqa: E402
from tools.mcp_tool_registration import (  # noqa: E402
...
class TestExtractReadOnlyHint:
    """Upstream's trust gate already owns the discovery-time capture, so T12 reads its record
    rather than keeping a second one. Its contract differs from the fork's in one way that
    matters here: it collapses to a plain bool, so "declared false" and "declared nothing" both
    read as False — write-capable is the fail-closed default for the gate. Only True is ever
    load-bearing, which is what these assert.
    """
    ...
    def test_dict_annotations(self):
        assert _annotation_read_only_hint(_tool("t", {"readOnlyHint": True})) is True

    def test_absent_or_unusable_is_never_true(self):
        assert _annotation_read_only_hint(_tool("t", None)) is False
        assert _annotation_read_only_hint(_tool("t", SimpleNamespace())) is False
        assert _annotation_read_only_hint(_tool("t", {"readOnlyHint": "yes"})) is False
...
class TestCaptureThroughRegistration:
    def test_hints_are_recorded_per_server_and_tool(self):
        ...
        # In the listing but declaring nothing: captured, and not read-only.
        assert mcp_tool_read_only_hint("hint_srv", "silent") is False
        # Never captured at all -> None, the accessor's "unknown" answer.
...
class TestReadOnlyHintCannotAutoApprove:
    ...
    def test_hint_true_does_not_change_the_elicitation_outcome(self, monkeypatch):
        ...
        monkeypatch.setattr(approval_context, "_get_approval_config", fake_config)
        monkeypatch.setattr(approval_context, "_is_gateway_approval_context",
                            lambda: False)
...
class TestHintReachesTheElicitationGate:
    def test_in_flight_tool_hint_is_passed_through(self):
        ...
        handler = ElicitationHandler("e2e_srv", {"timeout": 5}, in_flight_tool=lambda: server._pending_call_tool_name)
        seen = {}
        ...
        with patch("tools.approval_prompt.request_elicitation_consent", _record):
            result = asyncio.run(handler(context=None, params=_form_params()))
    ...
    def test_no_in_flight_tool_passes_none(self):
        ...
        handler = ElicitationHandler("e2e_srv2", {"timeout": 5}, in_flight_tool=lambda: server._pending_call_tool_name)
        seen = {}
    ...
    def test_audit_line_records_the_hint(self, monkeypatch, caplog):
        ...
        with caplog.at_level("INFO", logger="tools.approval"):
            approval_prompt.request_elicitation_consent(
                "confirm?", "server asks", read_only_hint=True,
            )
...
class TestEndToEndCaptureThroughDecision:
    ...
    def test_capture_and_a_real_call_drive_a_real_denial_with_the_hint_logged(
        ...
        from tools.mcp_tool import ElicitationHandler, _servers
        from tools.mcp_tool_handlers import _make_tool_handler
        import tools.approval_context as approval_context
        import tools.approval_prompt as approval_prompt
        ...
        handler = ElicitationHandler("e2e_full_srv", {"timeout": 5}, in_flight_tool=lambda: server._pending_call_tool_name)
        outcome = {}
