...
# tools.approval owns the session state and the guard entry points; the config readers live in
# tools.approval_context and the guardian LLM call in tools.approval_smart. Patch each name on
# the module that DEFINES it — both are resolved as module globals at call time, so a patch
# there reaches every consumer, including the ones tools.approval calls through.
...
class TestBarrierPrimitives:
    ...
    def test_aged_pending_is_not_open(self):
        ...
        with patch.object(approval_context, "_get_approval_timeout", return_value=0.001):
            import time
            time.sleep(0.005)
            assert session_has_open_human_decision("sess-a") is False
