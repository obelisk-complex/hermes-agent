...
# The normaliser, the getters and their module-level caches all live in
# tools.approval_context (the config-reader leaf). tools.approval re-exports the
# three getters, but NOT _last_known_good_auto_approve / _auto_approve_tags_warned:
# those are module state, and `from X import` copies the binding, so resetting them
# on the facade would silently miss the caches the getters actually write.
...
def _clean_state():
    ...
    approval_context._auto_approve_tags_warned.clear()
    yield
    approval_context._last_known_good_auto_approve = None
    approval_context._auto_approve_tags_warned.clear()
...
class TestNormalizeAutoApprove:
    ...
    def test_normalisation_table(self, value, expected):
        assert approval_context._normalize_auto_approve(value) == expected
...
class TestGetAutoApproveMode:
    def test_default_is_legacy(self):
        with patch.object(approval_context, "_get_approval_config", return_value={}):
            assert approval_context._get_auto_approve_mode() == "legacy"

    def test_read_failure_keeps_last_known_good(self):
        with patch.object(approval_context, "_get_approval_config",
                          return_value={"auto_approve": "dual_signal"}):
            assert approval_context._get_auto_approve_mode() == "dual_signal"
...
class TestGetAutoApproveTags:
    def test_empty_default(self):
        with patch.object(approval_context, "_get_approval_config",
                          return_value={}):
            assert approval_context._get_auto_approve_tags() == frozenset()
    ...
    def test_never_auto_approvable_dropped(self):
        with patch.object(approval_context, "_get_approval_config",
            ...
            tags = approval_context._get_auto_approve_tags()
