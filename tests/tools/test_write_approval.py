...
#
# The background split is driven by ``current_origin()`` after the Sep 2026
# decomposition (``is_background`` survives only in the plugin-compat block),
# so these patch the name ``evaluate_gate`` actually calls.
# ---------------------------------------------------------------------------
...
def test_memory_inline_deny_still_blocks_under_dual_signal(hermes_home, monkeypatch):
    ...
    _set_approval("memory", True)
    _set_dual_signal_all_tags()
    monkeypatch.setattr(wa, "current_origin", lambda: "foreground")
