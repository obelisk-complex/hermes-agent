...
# The Sep 2026 decomposition split these surfaces across three modules: tools.approval owns the
# session state and the two gate entry points, tools.approval_context the contextvars and config
# readers, tools.approval_prompt the CLI prompt and the elicitation gate. Some names are imported
# by value into tools.approval and some are called through the defining module, so patching a
# single target would silently miss one of the two surfaces under test here.
_PATCH_MODULES = (approval_module, approval_context, approval_prompt)


def _patch(monkeypatch, name, value):
    """Patch *name* on every approval module that binds it; fail if none does."""
    bound = [m for m in _PATCH_MODULES if hasattr(m, name)]
    assert bound, f"no approval module binds {name!r} — the patch would be a no-op"
    for module in bound:
...
def _clean_state(monkeypatch):
    ...
    approval_context._last_known_good_auto_approve = None
    _patch(monkeypatch, "get_current_session_key",
           lambda default="default": "phase-b-session")
    _patch(monkeypatch, "is_approved", lambda sk, pk: False)
    _patch(monkeypatch, "is_current_session_yolo_enabled", lambda: False)
    _patch(monkeypatch, "_YOLO_MODE_FROZEN", False)
    monkeypatch.setattr(
...
def _set_config(monkeypatch, auto_approve="dual_signal", mode="smart",
    ...
    _patch(monkeypatch, "_get_approval_config", fake_config)
...
class TestApprovalGateParity:
    ...
    def test_cli_outcome_unchanged_under_dual_signal(self, monkeypatch, choice,
        ...
        _patch(monkeypatch, "_is_interactive_cli", lambda: True)
        _patch(monkeypatch, "_is_gateway_approval_context", lambda: False)
        _patch(monkeypatch, "prompt_dangerous_approval", lambda *a, **k: choice)
        _patch(monkeypatch, "approve_session", lambda sk, pk: None)
    ...
    def test_cron_deny_outcome_unchanged_under_dual_signal(self, monkeypatch):
        _set_config(monkeypatch)
        _patch(monkeypatch, "_is_interactive_cli", lambda: False)
        _patch(monkeypatch, "_is_gateway_approval_context", lambda: False)
        _patch(monkeypatch, "_is_cron_approval_context", lambda: True)
        _patch(monkeypatch, "_get_cron_approval_mode", lambda: "deny")
    ...
    def test_no_human_still_fails_closed_under_dual_signal(self, monkeypatch):
        ...
        _patch(monkeypatch, "_is_interactive_cli", lambda: False)
        _patch(monkeypatch, "_is_gateway_approval_context", lambda: False)
        _patch(monkeypatch, "_is_cron_approval_context", lambda: False)
        res = request_tool_approval("terminal", "smtp send")
        assert res["approved"] is False
...
class TestApprovalGateNoAutoApproval:
    ...
    def test_dual_signal_gate_is_never_called(self, monkeypatch):
        ...
        _patch(monkeypatch, "_is_interactive_cli", lambda: True)
        _patch(monkeypatch, "_is_gateway_approval_context", lambda: False)
        _patch(monkeypatch, "prompt_dangerous_approval", lambda *a, **k: "deny")
...
class TestApprovalGateTags:
    ...
    def test_approved_result_carries_the_tag(self, monkeypatch):
        ...
        _patch(monkeypatch, "_is_interactive_cli", lambda: True)
        _patch(monkeypatch, "_is_gateway_approval_context", lambda: False)
        _patch(monkeypatch, "prompt_dangerous_approval", lambda *a, **k: "once")
        res = request_tool_approval("write_file", "writing ~/.ssh/authorized_keys")
    ...
    def test_yolo_short_circuit_shape_is_untouched(self, monkeypatch):
        ...
        _set_config(monkeypatch)
        _patch(monkeypatch, "is_current_session_yolo_enabled", lambda: True)
    ...
    def test_session_allowlist_short_circuit_shape_is_untouched(self, monkeypatch):
        _set_config(monkeypatch)
        _patch(monkeypatch, "is_approved", lambda sk, pk: True)
...
class TestElicitationParity:
    ...
    def test_prompt_exception_still_declines_under_dual_signal(self, monkeypatch):
        ...
        _patch(monkeypatch, "prompt_dangerous_approval", _raise)
...
class TestExecuteCodeParity:
    ...
    def _manual_mode(self, monkeypatch):
        ...
        """
        _patch(monkeypatch, "_get_approval_mode", lambda: "manual")
    ...
    def test_cron_approve_still_allows(self, monkeypatch):
        ...
        _patch(monkeypatch, "_is_gateway_approval_context", lambda: False)
        _patch(monkeypatch, "_is_cron_approval_context", lambda: True)
        _patch(monkeypatch, "_get_cron_approval_mode", lambda: "approve")
        assert check_execute_code_guard("print(1)", "local")["approved"] is True
    ...
    def test_session_allowlist_hit_still_allows(self, monkeypatch):
        _set_config(monkeypatch)
        _patch(monkeypatch, "_is_gateway_approval_context", lambda: True)
        _patch(monkeypatch, "_is_cron_approval_context", lambda: False)
        _patch(monkeypatch, "is_approved", lambda sk, pk: True)
        assert check_execute_code_guard("print(1)", "local")["approved"] is True
