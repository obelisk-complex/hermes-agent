...
class TestHermesConfigDetection:
    ...
    def _import(self):
        # The OWNER, not the tools.approval facade. `detect_dangerous_command`
        # reads DANGEROUS_PATTERNS(_COMPILED) as its own module globals, so
        # negative control 20 below — which patches those lists out to prove the
        # entry is what does the detecting — has to patch them where the function
        # looks them up. Patching the facade's re-exported copies changes nothing
        # and the control silently cannot fail.
        import tools.approval_detection as detection_module
        self._module = detection_module
