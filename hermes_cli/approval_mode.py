...
def _tags_listing_lines() -> list[str]:
    ...
    Header/dependency lines are added by the caller so the CLI and gateway surfaces can wrap the
    message differently. Reads live config so the listing reflects exactly what the gate enforces.
    """
    ...
    from tools.approval_context import _get_auto_approve_tags
    ...
    for tag in ActionTag:
        ...
        nature_s = (f"destructive={nature.destructive}, reversible={nature.reversible}, "
    ...
    try:
        ...
        if mode == "dual_signal":
            ...
            if isinstance(tags, list) and len(tags) == 0:
                warnings.append("dual_signal with no tags enabled: every previously "
                                "auto-approved action now races the approval timeout.")
            if str(approvals.get("mode", "smart") or "smart") != "smart":
                warnings.append("auto_approve only takes effect when approvals.mode is 'smart' — "
                                f"the current mode is '{approvals.get('mode')}'.")
            subagent = None
            if isinstance(cfg.get("delegation"), dict):
                subagent = cfg["delegation"].get("subagent_auto_approve")
            from utils import is_truthy_value
            if is_truthy_value(subagent):
                warnings.append("delegation.subagent_auto_approve is true — CLI-parented "
...
def _get_auto_approve_mode_for_listing() -> str:
    ...
    try:
        from tools.approval_context import _get_auto_approve_mode
...
def _command_allowlist_note() -> str:
    ...
    try:
        from hermes_cli.config import load_config_readonly
        entries = (load_config_readonly() or {}).get("command_allowlist") or []
    ...
    return (f"  command_allowlist: {count} entr{'y' if count == 1 else 'ies'} — these commands "
            "bypass the dual-signal tag gate entirely (approved above it at the allowlist check).")


def run_approval_tags_command(args: Optional[str]) -> ApprovalModeResult:
    """Handle ``/approvals tags`` (T10).

    ``args`` is the raw stripped argument substring after the command name, or ``None`` (D18).
    Subcommands: ``tags`` (listing), ``tags enable <tag>``, ``tags disable <tag>``. Only
    ``CONFIGURABLE_TAGS`` values are accepted; anything else is rejected without writing config.

    Write mechanism (D11): ``load_config()`` -> mutate the in-memory list -> ``save_config(config)``,
    mirroring ``save_permanent_allowlist`` — NOT ``set_config_value`` (which cannot write lists).
    """
    ...
    if not args:
        header = (f"Approval mode: {effective_mode} · auto_approve: "
                  f"{_get_auto_approve_mode_for_listing()}")
        msg = header + "\n" + "\n".join(_tags_listing_lines()) + "\n" + _command_allowlist_note()
    ...
    if verb not in ("enable", "disable"):
        return ApprovalModeResult(False, effective_mode, False,
                                  f"Usage: /approvals tags [enable|disable <tag>]. Valid tags: {valid}")
    if len(parts) < 2:
        return ApprovalModeResult(False, effective_mode, False, f"Usage: /approvals tags {verb} <tag>")
    tag = parts[1]
    ...
    # Managed config: save_config reports through managed_error and RETURNS without raising —
    # re-read to detect the silent non-write.
    ...
    return ApprovalModeResult(True, effective_mode, True,
...
def run_approval_mode_command(requested_mode: Optional[str]) -> ApprovalModeResult:
    ...
    if requested not in VALID_APPROVAL_MODES:
        return ApprovalModeResult(False, current, False, "Usage: /approvals [manual|smart|off|tags ...]")
