    ...
    *,
    ...
    with write_txn(conn):
        ...
        if model_override is not None:
            # Atomic escalation: fold the new model into the same status-flip
            # UPDATE so the requeue + model change commit together (no window
            # where the task is ready on the old model; crash-safe).
            cur = conn.execute(
