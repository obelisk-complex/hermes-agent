...
def complete_task(
    ...
    *,
    ...
    # Upstream's scratch-artifact discovery. Ordered AFTER the fork's
    # pre_kanban_complete gate and kept immediately before the status->done
    # write (upstream's own anchor): a gate plugin may create or remove files
    # in the scratch workspace, so the scan must see the workspace as the gate
    # left it, and a vetoed completion must not scan at all.
