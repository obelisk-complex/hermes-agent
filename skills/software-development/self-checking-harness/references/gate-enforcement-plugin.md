...
| `pre_verify` | Final text, before the loop breaks | Block a completion claim while a gate is open; honour `[GATE:ACCEPTING:<id>]`; verify citations; flag claimed-but-missing files; budget exhaustion → BLOCKED escalation |
...
- **Budget-exhaustion → BLOCKED escalation (v3.7.0; moved onto `pre_verify` in v3.8.0):** once the turn's `pre_verify` nudge budget (`agent.max_verify_nudges`, 5 in this fork) is spent and the gate is still open, `agent/turn_stop_gates.pre_verify_terminal_substitute` asks the plugin once more and replaces the final answer with an explicit BLOCKED-to-a-human message rather than shipping the unverified text.
- **Per-turn retry accounting (v3.6.0; core-owned since v3.8.0):** the retry count is upstream's per-turn `attempt`, reset by `agent/turn_context.py`'s per-turn state reset. It replaced the plugin's session-scoped counter, which kept climbing across turns and could fire the escalation text on a turn's first block. The iteration-budget exit re-checks a leaking claim via `agent/turn_finalizer.py`.
...
- **Strict plan↔artifact matching (v3.7.0, advisory):** claimed file creations/writes whose target is absent on disk are flagged via `pre_verify`.
- **Citation checker:** single-pass file-exists + line-in-range in `pre_verify`.
...
5. `pre_verify` blocks a direct completion claim (unless a clearance token is present); runs the citation + claimed-file checks; logs acceptances.
6. Once the turn's nudge budget (`agent.max_verify_nudges`) is spent → explicit BLOCKED escalation to a human.
...
| Version | Key change |
|---------|------------|
| v3.7.1 | Round-5 QA remediation: NEEDS_WORK enforced; done-claim bypasses closed; dead `_VERIFIES_TASK_RE` removed; `on_output` decision extracted to `agent/_on_output_gate.py` (superseded in v3.8.0); detection-first retained at `subagent_stop`; **plugin now bundled in the repo**; daily sync hardened (pinned `checkout`, 4-suite pre-push gate) |
