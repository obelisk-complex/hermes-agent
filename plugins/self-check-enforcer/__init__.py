"""Self-check enforcer plugin — v3.8.0: gate runs on upstream's pre_verify.
...
v3.8.0: migrated on_output -> upstream's pre_verify. The fork's parallel
        gate in agent/conversation_loop.py is gone; the same reject-and-retry
        now rides agent/turn_stop_gates.py. The retry budget is core's
        per-turn `attempt` against agent.max_verify_nudges, replacing this
        plugin's session-scoped counter (which could fire the escalation
        text on a turn's first block).
...
v3.7.2: on_output loop-fix via the documented token path (NO trigger narrowing,
      ...
      - as refusals accumulate within a turn, the rejection message
        escalates to a forcing directive once core's per-turn attempt count
        nears agent.max_verify_nudges: stop re-emitting a bare all-clear and resolve
...
# The retry budget is upstream's per-turn pre_verify bound (agent.max_verify_nudges),
# read live rather than mirrored as a constant: two thresholds that can disagree is
# how the ladder silently drifts out of step with the loop that enforces it. The
# fallback keeps this plugin importable stdlib-only — e.g. the bundled-plugin test
# loads it by file path, where the ``agent`` package may not be on sys.path — and
# matches upstream's DEFAULT_MAX_VERIFY_NUDGES.
# Terminal text when the retry budget is spent and the gate is still open. An
# explicit BLOCKED / escalate-to-a-human, not a vague "could not complete": the
# whole point is that the task is NOT silently marked done.
_BLOCKED_ESCALATION_MESSAGE = (
    "⚠️ BLOCKED — escalating to a human. The "
    "self-check gate blocked this output repeatedly "
...
try:
    from agent.verify_hooks import max_verify_nudges as _max_verify_nudges
except Exception:  # pragma: no cover - fallback when agent pkg not importable
    def _max_verify_nudges() -> int:
        return 3
...
# ── v3.8.0: pre_verify gate (was the fork's own on_output hook) ────────
...
def on_pre_verify(
    ...
    **kwargs: object,
    ...
    if not final_response or final_response.strip() == "(empty)":
        return None

    # ── Citation + claimed-artifact check (single pass here) ──
    citation_issues = _verify_citations(final_response)
    citation_issues = citation_issues + _verify_claimed_files(final_response)  # v3.7.0
    ...
    if state["pending_gate_violation"]:
        ...
        if state["pending_gate_violation"] and _claims_all_clear(final_response):
            ...
            # ``attempt`` is core's per-turn nudge count (0 on the first refusal),
            # so n is 1-based to match the wording below. The old session-scoped
            # counter kept climbing across turns and could fire the escalation text
            # on a turn's FIRST block.
            n = attempt + 1
            msg = (
            ...
            _budget = _max_verify_nudges()
            if attempt >= _budget:
                # Terminal call (agent.turn_stop_gates.pre_verify_terminal_substitute):
                # the nudge budget is spent and the gate is still open, so there is no
                # iteration left to retry into. This return replaces the final answer
                # rather than nudging — the escalation, not another retry prompt.
                return {"decision": "block", "reason": _BLOCKED_ESCALATION_MESSAGE}
            if n >= _budget - 1:
                remaining = max(0, _budget - attempt)
                msg += (
    ...
    # Allow path: gate not pending, or no all-clear claim this output. The retry
    # budget lives in core's per-turn ``attempt``, so there is nothing to reset.
    return None
