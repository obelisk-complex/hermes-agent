...
**Script QA before execution:** before running any script you wrote (shell, Python, etc.), get a multi-model/multi-agent QA panel review of it (see the `fleet-qa-loop` skill). Then dry-run it before letting it act for real. If the dry run misfires (errors, wrong side effects, unexpected diff), stop and loop fix, re-review, re-dry-run (QA-until-clean) until the dry run is clean, before the real run.
