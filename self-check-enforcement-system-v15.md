...
> ⚠️ **Authoritative source.** The source of truth is the installed plugin at `~/.hermes/plugins/self-check-enforcer/__init__.py`, the committed fork source, and the test suites — verify behaviour against those. The Python/YAML listings embedded below are regenerated verbatim from the live files by `regen-v15-embedded.py`; re-run it after any plugin change rather than hand-editing them (hand-drift was itself a v3.6.0 finding). Line numbers in design notes are informational; locate code by symbol.
...
description: "Auto-loads the self-checking-harness skill and enforces gate
  compliance. v3.7.1: round-5 remediation — NEEDS_WORK opens its own re-runnable
