...
core changes the gate rides on.

The system has four pieces:

1. **`pre_verify` gate widening**: upstream already ships the reject-and-retry
   mechanism (`agent/turn_stop_gates.py`, the `pre_verify` hook). It only fires
   on turns that edited a file, and it gives up silently when its nudge budget
   runs out. The fork widens the precondition so the gate also fires on
   non-editing turns, and substitutes an explicit BLOCKED message at exhaustion
   instead of letting the unverified answer through.
...
## 1. Apply the gate widening to the source

Three small edits to upstream files (`agent/turn_stop_gates.py`,
`agent/turn_final_response.py`, `agent/turn_finalizer.py`) plus the
`agent.max_verify_nudges` default, and nothing else. Apply them onto your
checkout:
...
git cherry-pick selfcheck/main   # 3-way apply; leaves one commit
```

Not a git checkout? Apply the diff directly instead:

```bash
curl -L "https://github.com/NousResearch/hermes-agent/compare/main...obelisk-complex:hermes-agent:main.diff" \
...
```bash
python3 -c "from hermes_cli.plugins import VALID_HOOKS; print('pre_verify' in VALID_HOOKS)"   # -> True
python3 -c "from agent.turn_stop_gates import pre_verify_terminal_substitute; print('terminal substitution present')"
```
...
them (via `pre_verify` + `subagent_stop`); this only closes the
...
- [ ] `"pre_verify" in VALID_HOOKS` is `True`
- [ ] `agent/turn_stop_gates.py` guards on `(_edited or api_call_count > 1)`,
      not `_edited` alone, and exports `pre_verify_terminal_substitute`
- [ ] `agent/turn_finalizer.py` consults that helper on the
...
- [ ] `plugins/self-check-enforcer/` present (`plugin.yaml` + `__init__.py`); the
      ...
      `on_session_start`, `on_session_end`, `pre_verify`
...
- [ ] direct success text is blocked by the `pre_verify` gate while a gate is open
