...
class SidebarCacheTests(unittest.TestCase):
    ...
    def test_projects_tree_coalesces_concurrent_scans_and_returns_copies(self):
        ...
        with mock.patch.object(profiles, "_profile_targets", return_value=[("default", Path("/nonexistent"))]), \
            ...
            # Only proving at least one of 8 pool workers got scheduled and reached the
            # coalescing lock, not measuring real latency — widened from 1s after CI flakes
            # under a loaded/oversubscribed runner (scheduling delay, not a code regression;
            # matches this fork's established precedent for shared-runner timing margins).
