...
def request_tool_approval(tool_name: str, reason: str, *, rule_key: str = "", approval_callback=None) -> dict:
    ...
    Gated branches also carry an ``action_tags`` key inherited from :func:`_run_approval_gate`
    (single-pattern tag resolution, Phase B T11). It records what class of action the decision
    covered, for audit/observability only: it is never an input to any approval decision here
